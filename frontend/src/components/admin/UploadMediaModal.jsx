import { useState, useRef } from 'react'; // --- UPDATED: Using useRef
...
const UploadMediaModal = ({ isOpen, onClose, onUpload, projectId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // --- NEW CHECK: Read file directly from ref ---
    ...
    try {
      ...
      // --- CRITICAL: Ensure correct field name ---
      formData.append('mediaFile', file);

      // Removed explicit headers block for file uploads
      ...
      setError(error.response?.data?.message || 'Failed to upload media. Check file type.');
    } finally {
      setLoading(false);
    }
  };

  const currentFileName = fileInputRef.current?.files?.[0]?.name || 'Click to upload';
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Upload Project Media" size="sm">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex justify-end gap-3 pt-4">
          ...
          {/* Check if a file is actually selected before enabling upload */}
          <Button type="submit" variant="primary" disabled={loading || !fileInputRef.current?.files?.[0]}>
