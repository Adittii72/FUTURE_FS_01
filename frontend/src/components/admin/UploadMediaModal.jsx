...
// //   const [uploadType, setUploadType] = useState('images'); // 'images' or 'video'
...
// //       setError('');
// //       setSelectedFiles([]);
// //       setUploadType('images'); // Default to images
...
// //   // Effect to update input attributes when type changes
// //   useEffect(() => {
// //     if (fileInputRef.current) {
...
// //           formData.append('imageFiles', file); // Field name must match backend
// //         });
// //       } else {
// //         formData.append('videoFile', selectedFiles[0]); // Field name must match backend
// //       }
...
const UploadMediaModal = ({ isOpen, onClose, onUpload, projectId }) => {
  ...
  // This hook runs when the modal opens
  useEffect(() => {
    if (isOpen) {
      setError('');
      setSelectedFiles([]);
      ...
      // --- THIS IS THE FIX ---
      // Manually set attributes for the default 'images' state
      // because the [uploadType] hook won't fire if the state doesn't change
      if (fileInputRef.current) {
        fileInputRef.current.accept = 'image/*';
        fileInputRef.current.multiple = true; // <-- This line fixes the bug
        fileInputRef.current.value = null;
      }
      // --- END FIX ---
    }
  ...
  // This hook runs when the user CLICKS the radio buttons to change type
  useEffect(() => {
    if (fileInputRef.current) {
      fileInputRef.current.accept = uploadType === 'images' ? 'image/*' : 'video/*';
      fileInputRef.current.multiple = uploadType === 'images';
        ...
        <div>
          ...
          <div className="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-6 text-center hover:border-purple-500 transition-colors">
            <input
              ...
              className="hidden"
              id="media-upload"
              // The 'multiple' and 'accept' attributes are now set by the useEffect hooks
            />
            <label
              htmlFor="media-upload"
              className="cursor-pointer flex flex-col items-center"
